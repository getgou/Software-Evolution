package android.app.printerapp.postPrinting.Tampering;
...
public class tamperingTestActivity extends AppCompatActivity implements Serializable, dialogComInterfaces.timeDialog, dialogComInterfaces.stringDialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tampering_test2);
    ...
    public void timeButtonPressed(View view) {
        ...
        bundleWithDelegate.putSerializable(enums.bundleKeys.dialogDelegate.getValue(), tamperingTestActivity.this);
        // Show time dialog
        TimePickerFragment<tamperingTestActivity> timePicker = new TimePickerFragment<>();
    ...
    public void fabPressed(View view) {
        ...
        LayoutInflater inflater = LayoutInflater.from(tamperingTestActivity.this);
        View dialogView = inflater.inflate(R.layout.activity_add_test_card_dialog, null);

        // Get a builder
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(tamperingTestActivity.this);
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        if (!(TextUtils.isEmpty(temp.getText())) && (!(TextUtils.isEmpty(time.getText())))) {
                            if (timeevent.matches("^(?:(?:([01]?\\d|2[0-3]):)([0-5]?\\d):)([0-5]?\\d)$")) {
                                ...
                                createNewTamperingTestDatabaseContent(temperature, timeevent, comment);
                            } else {
    ...
    public void getDatabaseContent() {
        ...
        // Get a call
        Call<List<TamperingTest>> call = endpoint.getTamperingTest(qrcode, token);

        // Enqueue the call
        call.enqueue(new Callback<List<TamperingTest>>() {
            @Override
            public void onResponse(Call<List<TamperingTest>> call, Response<List<TamperingTest>> response) {
                ...
                if (response.body() == null) {
                    ...
                    // Get the body
                    List<TamperingTest> data = response.body();
            ...
            @Override
            public void onFailure(Call<List<TamperingTest>> call, Throwable t) {
    ...
    public void updateDatabaseContent(String key, String newValue) {
        ...
        UpdateTampering dataToStore = this.getTamperingDataObject(key, newValue);
        ...
        // Get a call
        Call<String> call = endpoint.updateTamperingData(dataToStore, token);
    ...
    public void createNewTamperingTestDatabaseContent(String temperature, String time, String comment) {
        // Get the token
        String token = ((MyApplication) this.getApplication()).getToken();

        String qrcode = ((MyApplication) this.getApplication()).getQrcode();

        CreateTamperingTest data = new CreateTamperingTest();
        ...
        // Get a call
        Call<String> call = endpoint.createTamperingTest(data, token);
    ...
    public void createCards(List<TamperingTest> data) {
    ...
    private UpdateTampering getTamperingDataObject(String key, String content) {
        // Local object
        UpdateTampering data = new UpdateTampering();
